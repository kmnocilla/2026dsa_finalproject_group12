...
train_full <- train_full %>%
  mutate(
    ...
    date_planted = as.character(date_planted),
    date_harvested = as.character(date_harvested)
  )
...
corn_recipe_xgb <- recipe(yield ~ ., data = train_dev) %>%
  step_rm(grain_moisture, date_planted, date_harvested) %>%
...
pred_2024 <- predict(final_xgb_fit, test_full) %>%
...
pred_2024 <- predict(final_xgb_fit, test_full) %>%
  bind_cols(test_full %>% 
              select(site, year, hybrid)) %>%
  rename(predicted_yield = .pred)
```

#--------------SAVE PREDICTIONS------------
```{r}
write_csv(pred_2024, "../data/processed/predicted_yield_2024.csv")
```
